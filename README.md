...
##### Right-hand block drawer

###### Show right-hand block drawer on visit

With this setting on 'yes', the right-hand block drawer will be displayed in its expanded state by default. This only applies to users who are not logged in and does not overwrite the toggle state of each individual user.

###### Show right-hand block drawer on first login

With this setting on 'yes', the right-hand block drawer will be displayed in its expanded state by default. This only applies to users who log in for the very first time and does not overwrite the toggle state of each individual user.

###### Show right-hand block drawer on guest login

With this setting on 'yes', the right-hand block drawer will be displayed in its expanded state by default. This only applies to users who log in as a guest.
